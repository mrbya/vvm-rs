...
fn coverage_sampling_benches(criterion: &mut Criterion) {
    ...
    group.bench_function("coverpoint-and-cross", |bench| {
        bench.iter(|| {
            CoverageFixture::new("dut.coverage").map_or_else(
                |_| black_box(false),
                |mut fixture| black_box(fixture.sample_pair(4, 2).is_ok()),
            )
        });
    });
...
fn artifact_benches(criterion: &mut Criterion) {
    ...
    let Ok(artifact) = artifact_with_samples("artifact_bench", TestStatus::Passed, &pairs) else {
        return;
    };
...
fn merge_and_report_benches(criterion: &mut Criterion) {
    ...
    for artifact_count in [1_usize, 8, 64] {
        let Ok((merge, report)) = merge_fixture(artifact_count) else {
            continue;
        };
        ...
        throughput_artifacts(
            &mut group,
            u64::try_from(artifact_count).unwrap_or_default(),
        );

        group.bench_with_input(
            BenchmarkId::new("merge", artifact_count),
            &artifact_count,
            |bench, _| {
                let Ok(artifacts) = (0..artifact_count)
                    .map(|index| {
                ...
                bench.iter(|| {
                    black_box(CoverageMerge::from_artifacts(
        ...
        group.bench_with_input(
            BenchmarkId::new("merge-decode", artifact_count),
            &artifact_count,
            |bench, _| {
        ...
        group.bench_with_input(
            BenchmarkId::new("report-text", artifact_count),
            &artifact_count,
            |bench, _| {
        ...
        group.bench_with_input(
            BenchmarkId::new("report-html", artifact_count),
            &artifact_count,
            |bench, _| {
...
criterion_group!(
    coverage_benches,
    coverage_sampling_benches,
    artifact_benches,
    merge_and_report_benches
);
