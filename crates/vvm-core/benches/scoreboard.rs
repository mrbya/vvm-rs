...
fn scoreboard_benches(criterion: &mut Criterion) {
    ...
    group.bench_function("scalar/success", |bench| {
        bench.iter(|| {
            ...
            black_box(scoreboard.check(black_box(7_u32), black_box(7_u32)))
        });
    });
    ...
    group.bench_function("structured/success", |bench| {
        ...
        bench.iter(|| {
            let mut scoreboard = ExactScoreboard;
            black_box(scoreboard.check(black_box(expected.clone()), black_box(observed.clone())))
        });
    });

    for (name, observed) in [
        ("structured/early-mismatch", StructuredValue::new(0x55).early_mismatch()),
        ("structured/late-mismatch", StructuredValue::new(0x55).late_mismatch()),
    ] {
        group.bench_with_input(BenchmarkId::from_parameter(name), &observed, |bench, item| {
            let expected = StructuredValue::new(0x55);

            bench.iter(|| {
                let mut scoreboard = ExactScoreboard;
                let mismatch = scoreboard.check(black_box(expected.clone()), black_box(item.clone()));
                black_box(mismatch.err())
            });
        });
    }
