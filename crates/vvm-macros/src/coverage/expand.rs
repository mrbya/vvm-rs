...
pub(super) fn expand(input: Input) -> TokenStream {
    ...
    let constructors = fields.iter().map(|field| match *field {
        FieldRole::Coverpoint { ref ident, ref build, .. } => { let name = ident.to_string(); quote!(let #ident = #build(#name).map_err(|source| ::vvm::CoverageDefinitionError::Coverpoint { item: #name, source })?;) }
        FieldRole::Cross { ref ident, ref left, ref right, ref build } => { let name = ident.to_string(); build.as_ref().map_or_else(|| quote!(let #ident = ::vvm::Cross2::builder(#name, &#left, &#right).build().map_err(|source| ::vvm::CoverageDefinitionError::Cross { item: #name, source })?;), |builder| quote!(let #ident = #builder(#name, &#left, &#right).map_err(|source| ::vvm::CoverageDefinitionError::Cross { item: #name, source })?;)) }
    });
    ...
    quote! {
        ...
        impl ::vvm::__private::CoverageSpec for #model_ident {
            ...
            fn visit_coverage_items(&self, visitor: &mut dyn ::vvm::CoverageGroupVisitor) { #(#visits)* }
        }
        impl ::vvm::__private::CoverageSampleSpec<#stimulus, #observation> for #model_ident {
            fn sample_coverage_items(&mut self, cycle: ::vvm::ObservedCycle<'_, #stimulus, #observation>) -> ::core::result::Result<(), ::vvm::CoverageRuntimeError> { #(#samples)* #(#crosses)* ::core::result::Result::Ok(()) }
        }
    }
}
