...
fn metadata_decode_benches(criterion: &mut Criterion) {
    ...
    throughput_fixtures(
        &mut group,
        u64::try_from(FIXTURE_NAMES.len()).unwrap_or_default(),
    );
...
criterion_group!(
    metadata_benches,
    metadata_decode_benches,
    metadata_normalize_benches,
    codegen_benches
);
