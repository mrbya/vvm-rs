...
- Use `just benchmark`, `just benchmark-save-baseline <name>`, `just benchmark-compare-baseline <name>`, and `just benchmark-target <package> <target>` for local Criterion benchmarking. Baselines live under `target/criterion` and are machine-specific.
